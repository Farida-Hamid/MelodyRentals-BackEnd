...
RSpec.describe Reservation, :type => :model do
    before(:each) do
        @user = User.create!(name: 'Test User', username: 'testuser', email: 'user@example.com', password: 'password', role: 'regular')
        @instrument = Instrument.create!(name: 'Test Instrument', description: 'Test Description', image: 'Test Image', category: 'Test Category', price: 100, user_id: @user.id, available: true)
        @reservation = Reservation.create!(user_id: @user.id, instrument_id: @instrument.id, pickup_date: Date.today, return_date: Date.today + 1)
  end
